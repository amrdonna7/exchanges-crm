...
import { STAGES, LEAD_TYPES, PUBLISHERS, PRIORITIES } from '../lib/constants'
...
export default function NewLeadModal({ onClose, onCreated }) {
  ...
  const [form, setForm] = useState({
    ...
    city: '', type: '', publisher_interest: '', stage: 'prospect_qualifie',
    priority: 'normale', assigned_to: user?.id ?? '', notes: '',
  })
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    if (!form.organization_name.trim()) { setError('Le nom de l\'établissement est requis.'); return }
    setLoading(true)
    const { data, error: err } = await supabase
      .from('leads')
      .insert({ ...form, created_by: user?.id, assigned_to: form.assigned_to || null, custom_fields: {} })
      .select()
      .single()
    setLoading(false)
    if (err) { setError(err.message); return }
    await supabase.from('activities').insert({
      lead_id: data.id, user_id: user?.id, type: 'note', content: 'Prospect créé',
    })
    onCreated(data)
  }
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          <h2 className="font-bold text-slate-800 text-base">Nouveau prospect</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-3.5">
          {/* Name */}
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1.5">Établissement *</label>
            <input className="input" value={form.organization_name}
              onChange={e => set('organization_name', e.target.value)} placeholder="CASA - École Exemple" required />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1.5">Contact</label>
              <input className="input" value={form.contact_person}
                onChange={e => set('contact_person', e.target.value)} placeholder="Nom du contact" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input className="input" type="email" value={form.email}
                onChange={e => set('email', e.target.value)} placeholder="contact@ecole.ma" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1.5">Éditeur</label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1.5">Étape</label>
              <select className="input" value={form.stage} onChange={e => set('stage', e.target.value)}>
                {STAGES.map(s => <option key={s.id} value={s.id}>{s.label}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1.5">Priorité</label>
              <select className="input" value={form.priority} onChange={e => set('priority', e.target.value)}>
                {PRIORITIES.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1.5">Assigné à</label>
          ...
          <div>
            ...
            <textarea className="input resize-none h-16 text-sm" value={form.notes}
              onChange={e => set('notes', e.target.value)} placeholder="Notes initiales…" />
          </div>
