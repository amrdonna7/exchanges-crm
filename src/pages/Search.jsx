...
export default function Search() {
  ...
  async function performSearch(q) {
    ...
    const { data } = await supabase
      ...
      .select('*')
      .or(
        `organization_name.ilike.${term},contact_person.ilike.${term},email.ilike.${term},phone.ilike.${term},city.ilike.${term},notes.ilike.${term}`
      )
  ...
  return (
    <div className="p-4 lg:p-8 max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-500 text-sm mt-0.5">Search across all leads instantly</p>
      </div>
          ...
          {results.map(lead => {
            ...
            return (
              <Link
                key={lead.id}
                to={`/leads/${lead.id}`}
                className="card p-4 flex items-start gap-4 hover:border-brand-200 hover:shadow-md
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1 flex-wrap text-xs text-slate-500">
                    {lead.contact_person && (
                      <span>{highlight(lead.contact_person, debouncedQuery)}</span>
                    )}
                ...
                <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                  ...
                  {type && (
                    <span className="badge bg-slate-100 text-slate-600">{type.label}</span>
                  )}
                </div>
              </Link>
            )
      ...
      {!loading && !searched && (
        <div className="text-center py-16">
          ...
          <p className="text-slate-400 text-sm">Start typing to search all leads</p>
        </div>
      )}
    </div>
  )
